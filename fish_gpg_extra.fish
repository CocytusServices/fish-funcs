function fish_gpg_extra -d "Set GPG agent to GPG4Win agent"
  ...
  if test "$GPGA_ALREADY_RUNNING" -gt 0
    ...
    socat UNIX-LISTEN:"$GPG_AGENT_SOCK,fork" EXEC:"$HOME/npiperelay.exe -v -ep -ei -s -a $HOME/.gnupg/tmp-gpg-extra",nofork &
  end
end
